...
int main(int argc, char* argv[]) {
    ...
    for (int a = 0; a < 7; a++) {
        ...
        while (1) {
            ...
            //MEM
            mem_wb_latch[0] = mem_write(ex_mem_latch[1], &PCSrc, memory, &add_result, &(f_input->pre_alu_result), &forward_param, NULL, data_cache);
            ...
            // FETCH
            if_id_latch[0] = fetch(&pc, add_result, &PCSrc, memory, &count, NULL, inst_cache);
        ...
        printf("Num of store word              : %d\n", num_sw);
        printf("Num of load word               : %d\n", num_lw);
        printf("Num of instruction cache hit   : %d\n", inst_hit);
        printf("Num of data cache hit          : %d\n\n\n\n", data_hit);
